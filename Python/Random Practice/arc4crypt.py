...
# Key-scheduling algorithm (KSA)
    for i in range(256):
        print(f'Prev x: {x}', end = ", ")
        x = (x + box[i] + ord(key[i % len(key)])) % 256

        box[i], box[x] = box[x], box[i] # Swap values
        print(f'i={i}, x={x}, ibox[{i}] = {box[i]}, xbox[{x}] = {box[x]} key[{i} % len(key)]) = {key[i % len(key)]} , ascii = {ord(key[i % len(key)])}')
    ...
    # Pseudo-random generation algorithm (PRGA)
    for char in data:
        print(f'xbefore: {x}, ', end = "")
        x = (x + 1) % 256
        print(f'xafter: {x}, y before = {y} , box[{x}] = {box[x]}', end = ", ")
        y = (y + box[x]) % 256
        print(f'y after = {y},')
        box[x], box[y] = box[y], box[x] # Swap values
        out.append(chr(ord(char) ^ box[(box[x] + box[y]) % 256])) # XOR with keystream
